...
let timer = args[1];
let time = timer.replace(/y/, ' Yıl').replace(/d/, ' Gün').replace(/s/, ' Saniye').replace(/m/, ' Dakika').replace(/h/, ' Saat')

var date = new Date(Date.now())
var date2 = ms(timer)
var date3 = Date.now() + date2
...
kdb.add(`mutepuan.${member.id}.puan`, 10)
let number = await qdb.fetch('case')
if(number === null) number = "1"
  if(number === undefined) number = "1"
  moment.locale("tr")
kdb.push(`sicil.${member.id}`, {staff: message.author.id, user: member.id, ceza: 'MUTE', süre: timex, cezaid: number, date: tarih})
...
Bir kullanıcı metin kanallarında mutelendi!
...
client.channels.cache.get(voucher.Channels.MuteLog).send(new MessageEmbed().setDescription(`
Bir kullanıcının mutesi kalktı!
